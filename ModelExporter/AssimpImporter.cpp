...
void AssimpImporter::ShowNode(const aiNode& node)
{
	...
	PrintTabs(); std::cout << "Node Name : " << node.mName.C_Str() << std::endl;
	PrintTabs(); std::cout << "Children : " << nChildren << std::endl;
	PrintTabs(); std::cout << "Meshes : " << nMeshes << std::endl;
	for (int i = 0; i < nMeshes; ++i) {
		PrintTabs(); std::cout << "\tMesh #" << i<< std::endl;
		PrintMesh(*m_rpScene->mMeshes[node.mMeshes[i]]);
	}

	PrintTabs(); std::cout << "Transform : " << std::endl;
	PrintMatrix(node.mTransformation);

	PrintTabs(); std::cout << "MetaData? : " << (bool)node.mMetaData << std::endl;
	if (node.mMetaData) {
		aiMetadata* pMeta = node.mMetaData;
		for (int i = 0; i < pMeta->mNumProperties; ++i) {
			PrintTabs(); std::cout << '\t' << pMeta->mKeys[i].C_Str() << " : ";
...
void AssimpImporter::PrintMatrix(const aiMatrix4x4& aimtx)
{
	...
	PrintTabs(); std::println("\tR1 : [{: 5.3f}, {: 5.3f}, {: 5.3f}, {: 5.3f}]", xmf4x4Matrix._11, xmf4x4Matrix._12, xmf4x4Matrix._13, xmf4x4Matrix._14);
	PrintTabs(); std::println("\tR2 : [{: 5.3f}, {: 5.3f}, {: 5.3f}, {: 5.3f}]", xmf4x4Matrix._21, xmf4x4Matrix._22, xmf4x4Matrix._23, xmf4x4Matrix._24);
	PrintTabs(); std::println("\tR3 : [{: 5.3f}, {: 5.3f}, {: 5.3f}, {: 5.3f}]", xmf4x4Matrix._31, xmf4x4Matrix._32, xmf4x4Matrix._33, xmf4x4Matrix._34);
	PrintTabs(); std::println("\tR4 : [{: 5.3f}, {: 5.3f}, {: 5.3f}, {: 5.3f}]", xmf4x4Matrix._41, xmf4x4Matrix._42, xmf4x4Matrix._43, xmf4x4Matrix._44);
}

void AssimpImporter::PrintMesh(const aiMesh& aimesh)
{
	PrintTabs(); std::println("\t{{ HasBones : {} }}",					aimesh.HasBones());
	PrintTabs(); std::println("\t{{ HasFaces : {} }}",					aimesh.HasFaces());
	PrintTabs(); std::println("\t{{ HasNormals : {} }}",				aimesh.HasNormals());
	PrintTabs(); std::println("\t{{ HasPositions : {} }}",				aimesh.HasPositions());
	PrintTabs(); std::println("\t{{ HasTangentsAndBitangents : {} }}",	aimesh.HasTangentsAndBitangents());

	PrintTabs(); std::println("\t{{ NumUVChannels : {} }}",		aimesh.GetNumUVChannels());
	PrintTabs(); std::println("\t{{ NumColorChannels : {} }}",	aimesh.GetNumColorChannels());
